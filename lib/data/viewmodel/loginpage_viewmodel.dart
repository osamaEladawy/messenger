...
class LoginViewModel{
  ...
  signIN(BuildContext context,String toEmail,String toPassword) async {
    ...
    if(formState.currentState!.validate()){
      ...
      try {
        UserCredential userCredential = await authServise.sginINWithEmailAndPassword(
            toEmail, toPassword
        );
  ...
  forgetPassword(BuildContext context,String email)async{
    if(email == ""){
      AwesomeDialog(
        ...
        descTextStyle:const TextStyle(fontWeight: FontWeight.bold,color: Colors.white),
        barrierColor: Colors.white.withOpacity(0.8),
        context: context,
        title: "Haaaaaaye",
  ...
  Future<void> handleFacebookSignIn(BuildContext context,RoundedLoadingButtonController facebookController) async {
    final authServise = context.read<AuthServise>();
    ...
    if(internetServise.hasInternet == false){
      ...
      await authServise.signInWithFacebook().then((value){
        if(authServise.hasError == true){
          ...
          authServise.checkUerExists().then((value) async{
            if(value == true){
              ...
              await authServise.getUserDataFromFirestore(authServise.uid).then((value) => authServise.saveUserDataToSharedPreferences().then((value) => authServise.setSignIn().then((value){
              ...
              authServise.saveUserDataToFirestore().then((value) => authServise.saveUserDataToSharedPreferences().then((value) => authServise.setSignIn().then((value){
                if (kDebugMode) {
                  print(authServise.errorCode.toString());
                }
                if (kDebugMode) {
  ...
  Future<void> handlegoogleSignIn(BuildContext context,RoundedLoadingButtonController buttonController) async {
    final authServise = context.read<AuthServise>();
    //Provider.of<GoogleServise>(context, listen: false);
    final internetServise = context.read<InternetServise>();
    //Provider.of<InternetServise>(context, listen: false);
